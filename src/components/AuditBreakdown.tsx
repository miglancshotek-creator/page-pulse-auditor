...
const AuditBreakdown = ({ items }: AuditBreakdownProps) => {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold mb-4">{t("breakdown.title")}</h3>
      <div className="rounded-lg border border-border overflow-hidden">
        <div className="grid grid-cols-[1fr_80px_1fr] gap-4 p-3 bg-muted/50 text-xs font-medium text-muted-foreground uppercase tracking-wider">
          <span>{t("breakdown.category")}</span>
          <span className="text-center">{t("breakdown.score")}</span>
          <span>{t("breakdown.recommendation")}</span>
        </div>
        {items.map((item, i) => {
          const config = statusConfig[item.status as keyof typeof statusConfig] || statusConfig.warning;
          const Icon = config.icon;
          return (
            <div key={i} className="grid grid-cols-[1fr_80px_1fr] gap-4 p-4 border-t border-border items-start hover:bg-muted/20 transition-colors animate-fade-up" style={{ animationDelay: `${i * 100}ms` }}>
