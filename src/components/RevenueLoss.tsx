  ...
  return (
    <div data-pdf-section className="space-y-4 animate-fade-up">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-2xl font-bold">{t("revenue.title")}</h2>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-5 text-center">
          <p className="text-sm uppercase tracking-wider text-muted-foreground mb-1">{t("revenue.monthlyLoss")}</p>
          <p className="text-3xl font-extrabold text-red-400">{formatCurrency(totalMonthlyLoss)}</p>
        </div>
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-5 text-center">
          <p className="text-sm uppercase tracking-wider text-muted-foreground mb-1">{t("revenue.annualLoss")}</p>
          <p className="text-3xl font-extrabold text-red-400">{formatCurrency(totalAnnualLoss)}</p>
        </div>
      </div>
          ...
          <div key={i} className={`rounded-xl border p-4 ${severityStyles[item.severity]}`}>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-xs text-muted-foreground">{t("revenue.perMonth")}</p>
              </div>
            </div>
          </div>
        ))}
